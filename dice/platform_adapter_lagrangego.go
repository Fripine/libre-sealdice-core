...
var DefaultSignUrl = `https://sign.lagrangecore.org/api/sign`

func LoadSigInfo(filePath string) (*auth.SigInfo, error) {
...
func SaveSigInfo(filePath string, sigInfo *auth.SigInfo) error {
...
func LoadDevice(path string) *auth.DeviceInfo {
	data, err := os.ReadFile(path)
	if err != nil {
		deviceinfo := auth.NewDeviceInfo(int(crypto.RandU32()))
		_ = SaveDevice(deviceinfo, path)
		return deviceinfo
	}
	var dinfo auth.DeviceInfo
...
func SaveDevice(deviceInfo *auth.DeviceInfo, path string) error {
...
func FormatLagrangeGoElementGroup(elements []message.IMessageElement) []lagMessage.IMessageElement {
	...
	for _, element := range elements {
		...
		case *message.ImageElement:
			...
			segment = append(segment, &lagMessage.ImageElement{Stream: bytes.NewReader(data)})
...
func (pa *PlatformAdapterLagrangeGo) Serve() int {
	...
	if err := SaveDevice(deviceInfo, pa.configDir+"/deviceinfo.json"); err != nil {
		log.Errorf("Save DeviceInfo failed: %v", err)
	}

	// sigInfo, err := LoadSigInfo(pa.configDir + "/siginfo.bin")
	// if err != nil {
	// 	log.Errorf("Load SigInfo failed: %v", err)
	// 	log.Infof("Generating new SigInfo...")
	// 	pa.sig = auth.NewSigInfo(8848)
	// } else {
	// 	pa.sig = sigInfo
	...
	pa.QQClient = client.NewClient(pa.UIN, pa.signUrl, appInfo)
	pa.QQClient.UseDevice(deviceInfo)
	if sig, err := LoadSigInfo(pa.configDir + "/sig.bin"); err == nil {
		pa.QQClient.UseSig(*sig)
	}

	// data, err := os.ReadFile("sig.bin")
	// if err != nil {
	// 	log.Errorf("read sig error:", err)
	// } else {
	// 	sig, err := auth.UnmarshalSigInfo(data, true)
	// 	if err != nil {
	// 		log.Errorf("load sig error:", err)
	// 	} else {
	// 		pa.QQClient.UseSig(sig)
	// 	}
	// }
	...
	if err := pa.QQClient.Login("", pa.configDir+"/qrcode.png"); err != nil {
		log.Errorf("login err:", err)
		pa.EndPoint.State = 3
		pa.EndPoint.Enable = false
		return 1
	}
	...
	if err := SaveSigInfo(pa.configDir+"/sig.bin", pa.sig); err != nil {
...
func (pa *PlatformAdapterLagrangeGo) DoRelogin() bool {
	...
	if pa.QQClient != nil {
		...
		err := pa.QQClient.Login("", pa.configDir+"/qrcode.png")
		if err != nil {
